...
| 工作日早晨简报 | `morning-brief` | 工作日 | 过去约 24 小时 |
| 电算协同每日情报 | `compute-intel` | 工作日 | 过去约 24 小时 |
...
| 升学就业周报 | `education-weekly` | 周六 | 过去 7 天 |
...
| 对我们的意义 / 对家庭意味着什么 / 切入角度 | 非头条只显示 `brief`，所以写进 `brief` 的后半句，例如「……。意义：……」。头条写进 `summary` 的**第二句**，避免被截断。`selectionReason` 再完整写一遍，供存档 |
...
7. `sources[0]` 必须直接支撑标题和摘要的核心事实。能找到第二个独立来源时尽量附上（这样来源面板和英文原标题才会显示）。
8. 绝不编造链接、数字、价格、引语、日期。`publishedAt` 未知就省略。
9. 发布日期以官方首发为准（官网公告、版本说明、模型卡）。发布后的补充文章、评测、转载不能当作发布日期；若首发在窗口外，只能写成「某机构某日发文披露……」。
10. 行情数据先确认对应的交易日已经收盘，不引用尚未收盘的「收盘价」。
...
- 不写邮箱、账号、密钥等任何凭证。
- 电算协同每日情报按用户决定保留公司名「大云中联」和辽宁业务视角。

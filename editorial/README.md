...
- **全自动**：出刊过程中不向用户提问，一口气做完。拿不准的条目（来源打不开、日期或数字无法核实）直接不收；某板块没有可靠新闻就不写该板块；预检或长度提醒不通过时，自行修改候选稿后重跑。不使用 `--allow-warnings` 和 `--allow-history`。只有无法自行解决的故障（测试失败、构建失败、推送失败）才停下，并在最终报告里说明。所有取舍统一写进最终报告。
...
- **出刊即发布**（用户 2026-09-26 要求本地变动都同步到 GitHub）：「出刊」与「出刊并发布」一律提交并推送，运行出刊脚本时加 `--push`。
...
| 中文摘要 | `summary`（120–400 字，电算协同每日情报放宽到 650 字，见其手册；卡片会按版面截断，读者可点「展开全文」看完整内容，最重要的信息仍放前面）；`brief` 是能独立阅读的一句话（≤ 80 字）。标题上限：头条 42 字、重要 36 字、普通 28 字。超出时程序会给出长度提醒并影响版面，入库前压缩到上限内（按 Unicode 字符计，英文字母和空格也算） |
...
| 无法联网 / 检索整体失败 | 该刊物当天**不出**，不拿旧闻、不凭记忆凑数；其他能正常检索的刊物照常出。在报告中写明哪份没出、为什么 |
...
6. 出刊一律加 `--push`（见第 2 节「出刊即发布」）；推送后等部署完成，再核对线上页面。
...
- [ ] `process-candidate` 输出的 `warnings` 为空（长度提醒已处理；电算协同每日情报 summary 在 650 字以内的长度提醒除外）。
- [ ] 校验、测试、构建都通过，本机页面可打开。

向用户报告，用通俗中文，按固定三项写，不写技术过程（测试、构建等只在失败时说明）：

1. **出了哪几份**：每份一行，写刊物名、条数和可直接点开的网址（已推送给线上网址 `https://rich-richer.github.io/p/<id>/`，未推送说明「只在本机，尚未发布」）。没出的刊物也列出并写原因。
2. **没收的内容**：没收的条目或无更新的板块，每项一句话说明原因；没有就写「无」。
3. **需要你处理的事**：没有就写「无」。

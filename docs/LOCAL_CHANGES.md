...
**规则**：
- 刊物页正文上方加报头：英文眉题、刊名、双线日期栏（日期 · 星期、「事实优先 · 来源可查」、本期条数），翻期时随之更新。首页沿用原有标题区，只改样式。
- 导航不换行，放不下时横向滑动；导航里的刊名不再被截断。
- 配色、序号（Bodoni / Didot 数字，按阅读顺序 01、02……）、标题加粗、英文行斜体（Baskerville）只覆盖默认主题 `newspaper-default`。**没有新增主题**：原项目测试写死了主题库只有 3 个官方主题、首页使用默认主题，新增主题会导致测试和部署失败。切换到其他官方主题时这些外观不生效，报头与导航规则仍生效。
